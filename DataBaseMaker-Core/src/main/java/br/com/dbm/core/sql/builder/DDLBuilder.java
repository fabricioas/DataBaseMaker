...
public class DDLBuilder {
	...
	public static StringBuilder buildPK(Metadata metadata) throws Exception {
		...
		command.append(pk.getConstraint().getName()).append(" ON ");
		buildOwner(command, metadata.getTableOwner());
		command.append(metadata.getTableName()).append(" (").append(pk.getAttrName()).append(" ASC);\n\n");

		command.append("ALTER TABLE ");
		buildOwner(command, metadata.getTableOwner());
		command.append(metadata.getTableName()).append(" ADD CONSTRAINT ").append(pk.getConstraint().getName())
				.append(" PRIMARY KEY (").append(pk.getAttrName()).append(");\n");
		;
